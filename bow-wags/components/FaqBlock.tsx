...
export function FaqBlock({
  ...
  return (
    <section className={`font-bw-sans ${className}`}>
      {title ? (
        <div className="text-center">
          {eyebrow ? <Eyebrow>{eyebrow}</Eyebrow> : null}
          <h2 className="mt-1 font-bw-display text-2xl font-bold text-bw-ink sm:text-3xl">{title}</h2>
        </div>
      ) : null}
      <div className="mx-auto mt-8 max-w-3xl space-y-3">
        {items.map((item) => (
          <details key={item.question} className="group rounded-2xl border border-bw-border bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-sm text-bw-ink-soft leading-relaxed">{item.answer}</p>
          </details>
        ))}
      </div>
    </section>
