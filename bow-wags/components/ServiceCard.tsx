...
export function ServiceCard({
  ...
  return (
    ...
    >
      <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-bw-red/15 text-bw-red-dark">
        {icon}
      </div>
      <h3 className="font-bw-display text-lg font-bold text-bw-ink">{title}</h3>
      <p className="text-sm text-bw-ink-soft">{description}</p>
      <span className="mt-1 text-sm font-semibold text-bw-red-dark group-hover:underline">{cta} →</span>
    </Link>
  );
}
