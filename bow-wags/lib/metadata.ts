...
export function pageMetadata({
  ...
  // "%s | Bow Wags" template) — used for the homepage only.
  titleTemplate?: false;
}): Metadata {
  const url = `${SITE_URL}${path}`;
  const fullTitle = titleTemplate === false ? title : `${title} | Bow Wags`;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url,
      images: [{ url: OG_IMAGE, width: 1200, height: 630, alt: "Bow Wags" }],
    },
    twitter: {
