      ...
      <nav aria-label="Mobile" className="flex flex-col p-4 font-fh-sans">
        {groups.map((group) => (
          <div key={group.label} className="mb-2">
            <p className="mt-3 px-2 text-xs font-semibold uppercase tracking-wide text-fh-ink-soft">{group.label}</p>
  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="flex h-11 w-11 items-center justify-center rounded-lg border border-fh-border text-fh-ink"
      >
