...
export function StatBand({ className = "" }: { className?: string }) {
  return (
    <div
      className={`grid grid-cols-2 divide-x divide-y divide-psl-border rounded-3xl border border-psl-border bg-white shadow-sm sm:grid-cols-4 sm:divide-y-0 ${className}`}
    >
      {trustStats.map((s) => (
        <div key={s.label} className="flex flex-col items-center justify-center gap-1 px-4 py-6 text-center">
          <span className="font-psl-display text-2xl font-bold text-psl-brass-dark sm:text-3xl">
            {s.value}
          </span>
          <span className="text-xs font-semibold uppercase tracking-wide text-psl-ink-soft">
            {s.label}
          </span>
        </div>
      ))}
