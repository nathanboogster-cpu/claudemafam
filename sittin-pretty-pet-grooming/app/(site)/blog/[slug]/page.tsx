...
function BlockRenderer({ block }: { block: BlogBlock }) {
  if (block.type === "h2") {
    return <h2 className="mt-8 font-sp-display text-2xl font-bold text-sp-ink">{block.text}</h2>;
  }
  if (block.type === "list") {
    return (
      <ul className="mt-4 list-disc space-y-2 pl-5">
        {block.items.map((item) => (
          <li key={item} className="text-sp-ink-soft">
            {item}
          </li>
        ))}
      </ul>
  ...
  return <p className="mt-4 leading-relaxed text-sp-ink-soft">{block.text}</p>;
}
  ...
  return (
    <>
      ...
      <article className="mx-auto max-w-3xl px-4 py-12">
        ...
        <h1 className="mt-1 font-sp-display text-3xl font-bold text-sp-ink sm:text-4xl">{post.title}</h1>
        <time dateTime={post.publishedAt} className="mt-3 block text-sm text-sp-ink-soft">
        ...
        <div className="mt-10 flex flex-wrap gap-3 border-t border-sp-border pt-8">
      ...
      <section className="bg-sp-ink text-white">
        <div className="mx-auto flex max-w-3xl flex-col items-center gap-4 px-4 py-14 text-center">
          <h2 className="font-sp-display text-3xl font-bold">Ready to Schedule a Groom?</h2>
