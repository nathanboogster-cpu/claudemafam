...
export function ServiceCard({
  ...
  return (
    ...
    >
      <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-sp-green/15 text-sp-green-dark">
        {icon}
      </div>
      <h3 className="font-sp-display text-lg font-bold text-sp-ink">{title}</h3>
      <p className="text-sm text-sp-ink-soft">{description}</p>
      <span className="mt-1 text-sm font-semibold text-sp-green-dark group-hover:underline">Learn more →</span>
    </Link>
  );
}
