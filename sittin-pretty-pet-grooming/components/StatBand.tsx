...
export function StatBand({ className = "" }: { className?: string }) {
  return (
    <div
      className={`grid grid-cols-2 divide-x divide-y divide-sp-border rounded-3xl border border-sp-border bg-white shadow-sm sm:grid-cols-4 sm:divide-y-0 ${className}`}
    >
      {trustStats.map((s) => (
