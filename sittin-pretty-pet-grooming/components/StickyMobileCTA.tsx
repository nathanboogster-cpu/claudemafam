...
export function StickyMobileCTA() {
  ...
  return (
    <div
      ...
      inert={!visible}
      className={`fixed inset-x-0 bottom-0 z-30 flex gap-2 border-t border-sp-border bg-white p-2 transition-opacity duration-300 md:hidden ${
        visible ? "opacity-100" : "opacity-0"
      }`}
    >
      <CallButton location="sp_sticky_mobile_bar" variant="primary" label="Call Now" className="flex-1" />
      <SecondaryLinkButton
        location="sp_sticky_mobile_bar"
        variant="secondary"
        label="Our Services"
        href={PATHS.services}
        className="flex-1"
