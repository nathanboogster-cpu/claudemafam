...
// The only click event worth paying Redis round-trips for — the site's
// actual conversion CTA (calling to schedule), matching the event name
// already sent to Vercel Analytics in lib/track.ts. No online booking
// system exists, so a phone call is the only real conversion action.
const CLICK_TYPES = ["sp_call_click"] as const;
type ClickType = (typeof CLICK_TYPES)[number];

const CLICK_TYPE_LABELS: Record<ClickType, string> = {
  sp_call_click: "Call to Schedule",
};
...
// "hero" or "area_williamsport-md_cta" already, this just guards against
// anything unexpected reaching Redis.
