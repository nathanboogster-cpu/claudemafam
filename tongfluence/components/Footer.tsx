    ...
    <footer className="mt-20 border-t border-tf-border bg-tf-paper-deep text-tf-ink-soft">
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-12 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {socialProfiles.length ? (
            <ul className="mt-4 flex flex-wrap gap-4 text-sm">
              {socialProfiles.map((p) => (
                <li key={p.url}>
                  <a href={p.url} target="_blank" rel="noopener noreferrer" className="hover:text-tf-green-dark">
                    {p.label}
                  </a>
                </li>
              ))}
        ...
        <nav aria-labelledby="footer-services">
          <h2 id="footer-services" className="text-sm font-semibold uppercase tracking-wide text-tf-ink">
            What we do
          </h2>
          <ul className="mt-3 space-y-2 text-sm">
            {serviceNav.map((item) => (
              <li key={item.href}>
                <Link href={item.href} className="hover:text-tf-green-dark">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <nav aria-labelledby="footer-proof">
          <h2 id="footer-proof" className="text-sm font-semibold uppercase tracking-wide text-tf-ink">
            Case studies
          </h2>
          <ul className="mt-3 space-y-2 text-sm">
            <li>
              <Link href={PATHS.caseStudies} className="font-semibold hover:text-tf-green-dark">
                All case studies
              </Link>
            </li>
        ...
        <nav aria-labelledby="footer-company">
          <h2 id="footer-company" className="text-sm font-semibold uppercase tracking-wide text-tf-ink">
            Company
          </h2>
          <ul className="mt-3 space-y-2 text-sm">
            <li>
              <Link href={PATHS.about} className="hover:text-tf-green-dark">
                About Tongfluence
              </Link>
            </li>
            <li>
              <Link href={PATHS.book} className="hover:text-tf-green-dark">
                Book a call
              </Link>
            </li>
            <li>
              <Link href={PATHS.resources} className="hover:text-tf-green-dark">
                Resources
              </Link>
            </li>
            {resources.map((r) => (
              <li key={r.slug}>
                <Link href={`/resources/${r.slug}`} className="hover:text-tf-green-dark">
                  {r.navLabel}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-tf-border">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-5 text-xs sm:flex-row sm:items-center sm:justify-between">
          ...
          <ul className="flex gap-5">
            <li>
              <Link href={PATHS.privacy} className="hover:text-tf-green-dark">
                Privacy
              </Link>
            </li>
            <li>
              <Link href={PATHS.terms} className="hover:text-tf-green-dark">
                Terms
              </Link>
            </li>
          </ul>
